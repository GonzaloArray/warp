...
impl TuiTranscriptView {
    ...
    /// restored or post-bootstrap terminal block with displayed command or
    /// output rows (per [`should_render_terminal_block`] and
    /// [`block_content_rows`]; shell-bootstrap stages, the idle prompt block
    /// awaiting the first command, and zero-height blocks don't count). The
    /// session view fills the transcript slot with the zero state exactly while
    /// this holds.
